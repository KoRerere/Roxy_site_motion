...
// 处理滑块拖动
function handlePositionUpdate(clientX: number) {
  if (!sliderRoot.value)
    return
...
function handleMarkClick(mark: number) {
  if (mark >= props.min) {
...
<template>
  <div ref="sliderRoot" class="rx-slider" v-bind="$attrs">
    <!-- 渐变轨道 -->
    <div class="rx-slider__track__gradient" />
    ...
    >
      <div
        class="rx-slider__mark-label" :class="[
          mark === modelValue ? 'active' : '',
        ]"
      >
        {{ markFormatter(mark) }}
    ...
    >
      <RxIcon name="color/rx-rl-cane" size="36" class="absolute-center" />
      <div v-if="modelValue > 0" class="rx-slider__thumb-popup">
        {{ popupFormatter(modelValue) }}
      </div>
    </div>

    <div
      class="top-13 absolute -translate-x-50%"
      :style="{ left: `${Math.min(100 - maxWidth, mousePercent)}%` }"
      @mousedown="handleThumbDragStart"
    >
      <slot :max-width="maxWidth" :percent="mousePercent" :profile="modelValue" />
    </div>
  </div>
</template>
