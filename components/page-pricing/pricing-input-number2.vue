<template>
  <div class="inline-flex items-center gap-2 py-4 px-3 rounded-2 border-1 border-[#C7CDD1] border-solid">
    <span class="cursor-pointer flex items-center justify-center" @click="decrease">
      <RxIcon name="base/rx_ic_minus" color="#575D61" />
    </span>
    <input 
      :class="cn(
        'text-5 sm:text-6 bg-transparent border-0 outline-none text-center h-5 w-170px font-600 input-number',
        inputClass
      )" 
      ...
      @input="handleInput"
    />
    <span class="cursor-pointer flex items-center justify-center" @click="increase">
      <RxIcon name="base/rx_ic_plus" color="#575D61" />
    </span>
  </div>
</template>
