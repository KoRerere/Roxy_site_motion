...
const tableData = [
  ...
  {
    header: $t('RoxyBrowser'),
    headerIcon: 'functions/logo',
    headerClass: 'roxybrowser multilingual-text',
    ...
    items: [
      ...
      $t('绑定代理 IP+自动同步地理位置/语言/时区'),
      $t('原生集成 Selenium/Puppeteer，环境自动绑定'),
      $t('成员分权/操作审计/环境加密共享'),
      $t('封号率<2%，保障资产安全'),
      <NuxtLinkLocale to="/pricing" class={styles['to-pricing-btn']}>
          ...
          <div v-for="(item, index) in col.items" :key="item" class="cell" :class="{ hover: hoverIndex === index }" @mouseover="handleMouseOver(index)">
            <template v-if="isString(item)">
              <template v-if="col.itemIcon">
                <rx-icon-svg-icon :name="col.itemIcon" class="text-5 md:text-6" />
              </template>
              {{ item }}
            </template>
            <template v-else>
