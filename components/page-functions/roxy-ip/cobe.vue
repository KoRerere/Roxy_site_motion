...
/** 中文用「万」，其他语言用 M（1 万 = 0.01M = 10,000 IPs） */
...
function readGlobeLogicalSize(canvas: HTMLCanvasElement) {
  ...
  return w > 0 ? w : Math.min(500, globalThis.innerWidth - 32)
}
...
/** 仅渲染一帧：更新 globe 状态并同步 DOM 标签位置（不做持续 RAF 循环） */
...
/**
 * cobe 内部地图 PNG 异步 onload 后不会自动 update，单帧模式下需补渲染几次（非 RAF 循环）
 */
function ensureGlobeMapRendered() {
  renderGlobeFrame()
  requestAnimationFrame(() => {
    renderGlobeFrame()
    window.setTimeout(renderGlobeFrame, 120)
  })
}
...
async function initGlobe() {
  ...
  const globe = createGlobe(canvas, {
    ...
    glowColor: [1, 1, 1],
    offset: props.cobeLeft ? [-90, 60] : [90, 60],
    markers,
    arcs,
    arcColor: [0.5, 0.75, 1],
    arcWidth: 0.42,
  ...
  resizeObserver = new ResizeObserver(() => {
    ...
    if (next > 0) {
      ...
      // 尺寸变化后补一帧，保证 canvas 与标签对齐
      renderGlobeFrame()
    }
  })
  resizeObserver.observe(canvas)

  // 初始化补渲染至地图贴图就绪，避免持续 RAF
  ensureGlobeMapRendered()
}
...
<template>
  <div
    class="cobe-stack shrink-0 min-w-500px"
  ...
  >
    <div class="cobe-wrap relative">
      ...
      <div class="cobe-bg absolute z-1" />
    </div>
  </div>
</template>
...
.cobe-bg {
  ...
  height: 100%;
  right: 0;
  background: linear-gradient(144deg, #42a3fe -18.6%, #00d188 100%);
  z-index: 1;
  border-radius: 24px;
}
...
.cobe-orbit-ring {
  ...
  mask-image: linear-gradient(to bottom, rgb(0 0 0 / 10%) 40%, #000 48%, #000 100%);
  -webkit-mask-image: linear-gradient(to bottom, rgb(0 0 0 / 10%) 40%, #000 48%, #000 100%);
}
