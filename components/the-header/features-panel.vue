<template>
  <div class="flex flex-col text-white">
    <NuxtLinkLocale to="/features/window-sync">
      <div
        class="w-255px p-3 pb-4 bg-[radial-gradient(149.47%_89.09%_at_125.91%_100%,_#F9CDCE_0%,_#3384FF_100%)] rounded-1"
      >
        <div class="text-14px leading-18px font-600 font-[Archivo]">{{ $t('窗口同步') }}</div>
        <p class="text-12px font-400 mt-10px">
          {{ $t('充分减少重复任务负担，自动执行常规操作') }}
        </p>
      </div>
    </NuxtLinkLocale>
    ...
    <div class="mt-3 relative w-255px h-214px">
      ...
      <RxResponsiveImage name='header/profile-transfer' class="w-full" alt="" />
    </div>
  </div>
...
const coreFeatures = computed(() => [
  {
    ...
    link: "/features/profile-template",
  },
  {
